...
class Xcrud
{
    ...
    protected $advanced_params_html = "";
    ...
    protected $hide_logic = array();
    protected $disable_logic = array();
    protected $readonly_logic = array();
    ...
    public $labels = array();
    public $search = 0;
    public $limit_add_records = ""; //if you set to "" it will not check limit
    ...
    public function limitAddRecords($limit) {
        
        $this->limit_add_records = $limit;
        return $this;
    }
    ...
    protected function get_query()
    {
        ...
        if (!empty($this->_select_fields)) {
            $query .= implode(", ", $this->_select_fields);
        } else {
            $query .= "*";
        }
    ...
    public function set_logging($bool = true,$user_id = "",$role_id = "")
    {
        $this->is_log = (bool)$bool;
        $this->log_user_id = $user_id;
        $this->log_role_id = $role_id; 
        return $this;
    }
        ...
        if ($type)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                switch ($type)
                {
                    ...
                    case 'multiselect':  
                    case 'radio':
                          //$attr = array('attributes_additional' => $attr);
    ...
    public function hide_logic($fields = '',$fields2 = '', $operator = '', $value = '', $color = '', $class = '')
    {
        $uniqueId = uniqid("");
        $this->set_attr("$fields",array("data-operator"=>"$operator","data-display-logic"=>"$fields2","data-depend-value"=>"$value"));
        $this->set_attr("$fields2",array("data-display-logic-src"=>"$fields2"));

        //$this-set_attr("$fields2","F1");
        if ($fields && $fields2 && $operator && $value)
        {
            ...
            foreach ($fdata as $fkey => $fitem)
            {
                if (isset($this->field_attr[$fkey]))
                {
                    if (isset($attr)) {
                         ...
                         $this->field_attr[$fkey] = (array )$this->field_attr[$fkey];
                    }
                }
                else
                {
    ...
    public function disable_logic($fields = '',$fields2 = '', $operator = '', $value = '', $color = '', $class = '')
    {
        $uniqueId = uniqid("");
        $this->set_attr("$fields",array("data-operator"=>"$operator","data-disable-logic"=>"$fields2","data-depend-value"=>"$value"));
        $this->set_attr("$fields2",array("data-disable-logic-src"=>"$fields2"));
    ...
    public function readonly_logic($fields = '',$fields2 = '', $operator = '', $value = '', $color = '', $class = '')
    {
        $uniqueId = uniqid("");
        $this->set_attr("$fields",array("data-operator"=>"$operator","data-readonly-logic"=>"$fields2","data-depend-value"=>"$value"));
        $this->set_attr("$fields2",array("data-readonly-logic-src"=>"$fields2"));
    ...
    private function advanced_params_html($html = '')
    {
        $this->advanced_params_html = $html;
        return $this;
    }
    ...
    public function panel_view($status,$columns_per_row){
        $this->panel_view = $status;
        $this->columns_per_row = $columns_per_row;
        return $this;
    }
    ...
    public function _render_file_xcrud()
    {
    ...
    protected function _insert($postdata, $no_processing = false, $no_processing_fields = array())
    {
        ...
        if (!$this->demo_mode && $this->is_log == true){
            ...
            $log_user_id = 0;
            if(isset($log_user_id)){
                $log_user_id = $this->log_user_id();
            }

            $log_role_id = 0;
            if(isset($log_role_id)){
                $log_role_id = $this->log_role_id();
            }

            $q_log = 'INSERT INTO `' . $this->table . '` (' . implode(',', array_keys($set[$this->table])) . ') VALUES (' .
                implode(',', $set[$this->table]) . ')'; 
            $db->query('INSERT INTO `logs` (record_id,action,table_name,data,updated_by,sys_role_id) VALUES ("' . $ins_id . '","INSERT","' . $this->table . '","' . $q_log . '","' . $log_user_id . '","' . $log_role_id . '")');
        }   
    ...
    protected function _update($postdata, $primary)
    {
        ...
        if (!$this->demo_mode && $this->is_log == true){
            ...
            $q_log = "UPDATE `{$this->table}` AS `{$this->table}` " . implode(' ', $joins_qr) . " SET " . implode(",\r\n", $set) .
                    " WHERE `{$this->table}`.`{$this->primary_key}` = " . $db->escape($primary);    
            $db->query('INSERT INTO `logs` (record_id,action,table_name,data,old_record,updated_by,sys_role_id) VALUES (' . $db->escape($primary) . ',"UPDATE","' . $this->table . '","' . $q_log . '","' . $row_log_json . '","' . $log_user_id . '","' . $log_role_id . '")');
        }   
    ...
    {
         $db = Xcrud_db::get_instance($this->connection);
        //print_r($this->primary_val);
        if (is_array($this->primary_val)) {

            foreach ($this->primary_val as $valueP) {
               
                //$escapedValue = $db->escape($valueP);
                //$inClause .= "$escapedValue, ";
                $del = false;
                if ($this->table_ro)
                    return self::error('Forbidden');
                if ($this->before_remove)
                {
                    ...
                    {
                        call_user_func_array($this->before_remove['callable'], array($valueP, $this));
                        if ($this->exception)
                        {
                            $this->task = 'list';
                            $this->primary_val = null;
                ...
                if ($this->replace_remove)
                {
                    ...
                    {
                         echo "xxxc";
                        $this->primary_val = call_user_func_array($this->replace_remove['callable'], array($valueP, $this));
                    }
                }
                else
                {
                    ...
                    $db->query("SELECT * FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($valueP) . " LIMIT 1");
                    //echo "SELECT * FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape() . " LIMIT 1";
                    $row_log = $db->row();
                    ...
                    if (!$this->join)
                    {
                        if ($fields)
                        {
                            $db->query('SELECT ' . implode(',', $fields) . " FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($valueP) . ' LIMIT 1');
                            $del_row = $db->row();
                        }
                        if (!$this->is_remove($del_row))
                            return self::error('Forbidden');
                        if (!$this->demo_mode)
                            $del = $db->query("DELETE FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($valueP) .
                                " LIMIT 1");
                        ...
                        if (!$this->demo_mode && $this->is_log == true){
                            ...
                            $q_log = "DELETE FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($valueP) .
                                " LIMIT 1"; 
                            $db->query('INSERT INTO `logs` (record_id,action,table_name,data,old_record,updated_by,sys_role_id) VALUES (' . $db->escape($this->primary_val) . ',"DELETE","' . $this->table . '","' . $q_log . '","' . $row_log_json . '","' . $log_user_id . '","' . $log_role_id . '")');
                        }           
                    }
                    else
                    {
                        ...
                        if ($fields)
                        {
                            $db->query('SELECT ' . implode(',', $fields) . " FROM `{$this->table}` AS `{$this->table}` " . implode(' ', $joins) .
                                " WHERE `{$this->table}`.`{$this->primary_key}` = " . $db->escape($valueP));
                            $del_row = $db->row();
                        }
                        if (!$this->is_remove($del_row))
                            return self::error('Forbidden');
                        ...
                        if (!$this->demo_mode && $this->is_log == true){
                            ...
                            $q_log = "DELETE " . implode(',', $tables) . " FROM `{$this->table}` AS `{$this->table}` " . implode(' ', $joins) .
                                " WHERE `{$this->table}`.`{$this->primary_key}` = " . $db->escape($valueP);  
                            $db->query('INSERT INTO `logs` (record_id,action,table_name,data,old_record,updated_by,sys_role_id) VALUES (' . $db->escape($valueP) . ',"DELETE","' . $this->table . '","' . $q_log . '","' . $row_log_json . '","' . $log_user_id . '","' . $log_role_id . '")');
                        }   
                    }
                ...
                if ($this->after_remove)
                {
                    ...
                    {
                        call_user_func_array($this->after_remove['callable'], array($valueP, $this));
                    }
                }
            }

        }else{//only one primary key
    ...
    protected function _build_where()
    {
        ...
        if ($this->search && ($this->task == 'list' or $this->task == 'print' or $this->task == 'csv' or $this->after == 'list'))
        {
            ...
            if (is_array($this->advanced)) {
                ...
                {       
                    ...
                    elseif (isset($this->subselect[$key]))
                    {
                        if($value !=""){
                            ...
                            {
                               if($where_arr){
                                    if($where_arr[count($where_arr)-1] == "AND"){
                ...
                $advanced_params .= "</div>";
                $this->advanced_params_html = $advanced_params;
                //print_r($where_arr);
            }
            else
            {
        ...
        if(isset($where_arr[count($where_arr)-1])){
            if($where_arr[count($where_arr)-1] == "AND"){
                unset($where_arr[count($where_arr)-1]); 
            }
        }
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        else
        {
            ...
            {             
                ...
                $posted_advanced = $this->_post('advanced_search');
                if(is_array($posted_advanced)){
                    $this->advanced = $posted_advanced;
                }

                if(count($this->advanced) > 0){
    ...
    protected function create_save_and_edit($name, $value = '')
    {
        ...
        return '<button val="' . $value . '" type="button" class="btn btn-info" onclick=Xcrud.request(".xcrud-ajax",Xcrud.list_data(".xcrud-ajax",{task:"save",after:"edit"}),false,3);>' . $this->labels[$name] . '</button>';
    }
	protected function create_save_and_return($name, $value = '')
    {
        //echo $name . "<br>" . $value . "<br>";  
        return '<button val="' . $value . '" type="button" class="btn btn-secondary" onclick=Xcrud.request(".xcrud-ajax",Xcrud.list_data(".xcrud-ajax",{task:"save",after:"list"}),false,3);>' . $this->labels[$name] . '</button>';
    }
    ...
    protected function create_price($name, $value = '', $tag = array())
    {
       if (isset($this->subselect[$name]))
        ...
        $tag = $tag + array(
            ...
            'name' => $name,
            //'oninput'=> "Xcrud.formatPrice(this)",
            'data-pattern' => 'numeric');
        ...
        return $this->single_tag($tag, "",$price_val, true) . "";
    }
    ...
    protected function create_button($name, $value = '', $tag = array())
    {
        ...
        //$att = $this->field_attr[$name]["attributes_additional"];
        ...
        /*$tag = $tag + array(
            'tag' => 'a',
            'class' => !empty($attr['class']) ? $attr['class'] : null,
            'href' => !empty($href) ? $href : null,
            'data-after' => !empty($after) ? $after : null,
            'data-task' => !empty($task) ? $task : null,
            'data-action' => !empty($action) ? $action : null,
            'data-primary' => !empty($this->primary_val) ? $this->primary_val : null,
        ...
        $attributes = array(
            ...
            'data-task' => $task,
            'data-action' => $action,
            'data-primary' => $this->primary_val,
            'title' => $title
        );

        $difference = array_diff($text_val, $attributes);
        ...
        $tagString .= '><i class="' . $icon .  '"></i> ' . $title . ' </' . $tag['tag'] . '>';

        return $tagString;
    }

    protected function create_flatpickr($name, $value = '', $tag = array())
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_text($name, $value);
        ...
        //print_r($text_val);
        $out = $this->single_tag($tag, $this->theme_config('text_field'), $text_val, false);

      /*$out =  <input class='x xcrud-input form-control flatpickr-input' type='text'
data-type='text' flatpickr='" . $element_id . "' value='" . $value . "' name='" . $name . "' attributes_additional='{
        enableTime: true,
        dateFormat: &quot;Y-m-d H:i&quot;,
    }'>*/

        $out .= "<script>
                var element_ = document.querySelector('input[flatpickr=\"$element_id\"]');
                flatpickr(element_, $att);
                $(element_).prop('readonly', false); 
                 </script>";
        ...
        /*$out .= "<script>
            ...
            flatpickr(element_, {
                onChange: function(selectedDates, dateStr, instance) {
                    // Convert the selected date to Unix timestamp format
                    var timestamp = Math.floor(new Date(dateStr).getTime() / 1000);

                    // Update the value of the input field with the timestamp
                    element_.value = timestamp;
                }
            });
            $(element_).prop('readonly', false); 
         </script>";*/
    ...
    protected function create_view_switch($name, $value = '', $tag = array())
    {
        if (Xcrud_config::$clickable_list_links)
        {
            $value = $this->make_links($value);
    ...
    protected function create_view_range($name, $value = '', $tag = array())
    {
        if (Xcrud_config::$clickable_list_links)
        {
            $value = $this->make_links($value);
    ...
    protected function create_drawing($name, $value = '', $tag = array())
    {
        $element_id = rand(10,1000);
        $out = "";
        $opt_tag = array('tag' => 'canvas', 'width' => '200','height' => '200','id'=>"sig-canvas$element_id","class"=>"sig-canvas");
        ...
        $out .= $this->open_tag($opt_tag) . $this->html_safe("Not supported by browser") . $this->close_tag($opt_tag);
        $out .= $this->single_tag($tag + array(
            'tag' => 'input',
            'type' => 'hidden',
            'id' => "sig-dataUrl$element_id",
            'value' => $value,
         ...
         if($value != ""){
            ...
            $out .= $this->single_tag($tag + array(
                        'tag' => 'img',
                        'id' => "sig-image$element_id",
                        'width' => '200',
                        'height' => '200',
                        'style' => 'float:right',
                        'src' => $value), 'xcrud-input',array(), true);
                        $out .= "</div>";
         }
         ...
         $out .= "<script> document.addEventListener('DOMContentLoaded', function() {
            ...
            clearCanvas();
            initializeSignatureCanvas('sig-canvas" . $element_id . "'); // Reinitialize the canvas
            }, false);
        }); initializeSignatureCanvas($element_id);</script>";

        $out .="<div class='row'>
			<div class='col-md-12'><button type='button' class='btn btn-default' id='sig-clearBtn$element_id' onclick='clearCanvas($element_id);';>Clear Signature</button></div></div>";
        return $out;
    }

    protected function create_view_drawing($name, $value = '', $tag = array())
    {
        $element_id = rand(10,1000);
        $out = "";
        $opt_tag = array('tag' => 'canvas', 'width' => '200','height' => '200','id'=>"sig-canvas$element_id","class"=>"sig-canvas");
   ...
   protected function create_switch($name, $value = '', $tag = array())
{
        $checkName = explode(".",$name);
        $cntName = count($checkName);
        $name_old = "";
        ...
        $label_tag = array('tag' => 'label', 'class' => 'xcrud-checkboxes-label switch');
        ...
        if (is_array($this->field_attr[$name]['values']))
        {
            foreach ($this->field_attr[$name]['values'] as $optkey => $opt)
            {
                ...
                $out .= $this->html_safe($opt) . "<span class='slider'></span>" . $this->close_tag($label_tag) . $this->close_tag('div');
            }
        }
        else
        {
    ...
    protected function create_radio_buttons($name, $value = '', $tag = array())
    {
        ...
        if (is_array($this->field_attr[$name]['values']))
        {
            foreach ($this->field_attr[$name]['values'] as $optkey => $opt)
            {
                ...
                $random_no  = rand(10,100000);
                $label_tag = array('tag' => 'label', 'class' => 'xcrud-radio-label', 'for' => "$random_no");

                $out .= $this->open_tag('div', $this->theme_config('radio_container'));
                $attr = array('value' => $this->html_safe($opt),'id' => $random_no);
                //$attr = array('id' => $random_no);
                //$attr = array('value' => $this->html_safe($opt));
                $label_attr = array('for' => $random_no);
                if ($optkey == $value)
                {
                    $attr['checked'] = '';
                }
                $out .= $this->single_tag($tag, $this->theme_config('radio_field'), array_merge($this->field_attr[$name], $attr), true);
                $out .= $this->open_tag($label_tag) . $this->html_safe($opt) . $this->close_tag($label_tag) . $this->close_tag('div');
            }
        }
        else
        {
            ...
            foreach ($tmp as $opt)
            {
                ...
                $out .= $this->open_tag('div', $this->theme_config('radio_container'));
                //$attr = array('value' => $opt);
                $attr = array('id' => $random_no,'value' => $this->html_safe($opt));
                //$attr = array('value' => $this->html_safe($opt));
                if ($opt == $value)
                {
                    $attr['checked'] = '';
        ...
        $main_radio_v2_tag = array('tag' => 'div', 'class' => 'radio_v2');
        $out = $this->open_tag($main_radio_v2_tag) . $out . $this->close_tag($main_radio_v2_tag);
        return $out;
    }
    ...
    protected function create_range($name, $value = '', $tag = array())
    {
        ...
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_text($name, $value, $tag);
        }
        ...
        $tag = $tag + array(
            ...
            'max' => $max,
            'value' => $value,
            'oninput' => "updateValue(this.value , '$name')",
            'name' => $name);
        ...
        $out .= "<span id='rangeValue$name'>$value</span><div class='container'><span class='min'>$min</span>" . $this->single_tag($tag, $this->theme_config('text_field'), $text_val, true) . "<span class='max'>$max</span></div>";
        $out .= "<script>function updateValue(value,un) {
          console.log(value);
          console.log(un);
          document.getElementById('rangeValue$name').textContent = value;
        }</script>";
        return $out; 
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
    {
        ...
        $cleanNumberTmp = preg_replace( '/[^0-9]/', '', sha1($this->instance_name . ($filename ? $filename : rand())));
        $cleanNumber = base_convert($cleanNumberTmp, 10, 36);
    ...
    public static function load_css()
    {
        ...
        if (Xcrud_config::$load_canvas_draw)
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/canvas/xcrud-drawings.css" rel="stylesheet" type="text/css" />';
        if (Xcrud_config::$enable_flatpickr)
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/flatpickr/css/flatpickr.min.css" rel="stylesheet" type="text/css" />';
        if (Xcrud_config::$load_additional_elements)
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/additional-elements/css/additional-elements.css" rel="stylesheet" type="text/css" />';
        ...
        $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
            ...
            '/' . Xcrud_config::$themes_uri . '/' . Xcrud_config::$theme . '/xcrud.css?v1.0" rel="stylesheet" type="text/css" />';
    ...
    public static function load_js()
    {
        ...
        {
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/canvas/xcrud-drawing.js"></script>';
        }
        if (Xcrud_config::$load_bootstrap4)
        {
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
        ...
        if (!is_array($value) && !Xcrud_config::$performance_mode && $value)
        {
            foreach ($data as $key => $val)
            {
                ...
                if (!is_array($val))
                {
                    ...
                    $t = $safety ? $this->html_safe($val) : $val;
                    if($t == "" || $value == "" || $key == ""){
                        
                    }else{
                        $value = str_ireplace('{' . $key . '}', $t, $value);
                    }
 ...
 'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
{
         ...
         if ($html_template != "") {
            ...
            // Loop through fields_output to replace placeholders in the template
            foreach ($this->fields_output as $key => $item) {

                $arr = explode(".",$item['name']);
                $n = $arr[1];
                // Replace {fieldname} with actual field element
                $html_template = str_replace("{" . $n . "}", $item['field'], $html_template);
            }
            //Set the processed HTML template as the output      
            //$html_template = str_replace("{tabs}", $out_tabs, $html_template);
                     ...
                     if (isset($this->field_tabs[$mode]) or $this->default_tab !== false)
                     {
                         ...
                         if ($this->default_tab !== false && $raw_out)
                         {
                             ...
                             $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->
                                 ...
                                 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid,'tab-name' => $tabname)) . $this->default_tab . $this->
                         ...
                         if (isset($this->field_tabs[$mode]) && $tabs_out)
                         {
                             foreach ($this->field_tabs[$mode] as $key => $tabname)
                             {
                                 if (isset($tabs_out[$tabname]))
                                 {
                                     ...
                                     $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->
                                         theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k ==
                                         0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid,'tab-name' => $tabname)) . $tabname . $this->close_tag($tabs_link) .
    ...
    protected function render_search()
    {
        ...
        /*if(!$this->advanced_search_active){//disable simple search when advanced search is activated
            return;
        }*/
    ...
    protected function render_grid_body_old_($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td')) {
        $out = '';
        $i = 0;
    
        // Check if there are grouping columns defined.
        if (count($this->group_by_columns) > 0) { 
            // Define arrays to store unique values for each level of grouping.
            $unique_group_array_values = array();
            $unique_group_array_values_level2 = array();
            $unique_group_array_values_level3 = array();
        
            // Iterate over the result list to extract grouping values.
            foreach ($this->result_list as $key_group => $row_group) {
                // Extract values for each level of grouping.
                foreach ($this->group_by_columns as $level => $group_column) {
                    $field = $group_column['table'] . "." . $group_column['field'];
                    $group_value = $row_group[$field];
                
                    // Store unique values for each level of grouping.
                    if ($level == 0) {
                        array_push($unique_group_array_values, $group_value);
                    } elseif ($level == 1) {
                        array_push($unique_group_array_values_level2, $group_value);
                    } elseif ($level == 2) {
                        array_push($unique_group_array_values_level3, $group_value);
                    }
                }
            }
        
            // Perform grouping and rendering logic.
            // Iterate over unique values for the first level of grouping.
            foreach ($unique_group_array_values as $key_group_level1) {
                // Output the first level group header.
                $out .= $this->open_tag($row_tag, 'xcrud-row xcrud-grouping-row-level1') . $this->open_tag($item, '', array('colspan' => count($this->columns))) . $key_group_level1 . $this->close_tag($item) . $this->close_tag($row_tag);

                // Iterate over unique values for the second level of grouping.
                foreach ($unique_group_array_values_level2 as $key_group_level2) {
                    // Output the second level group header.
                    $out .= $this->open_tag($row_tag, 'xcrud-row xcrud-grouping-row-level2') . $this->open_tag($item, '', array('colspan' => count($this->columns))) . $key_group_level2 . $this->close_tag($item) . $this->close_tag($row_tag);
                
                    // Iterate over unique values for the third level of grouping.
                    foreach ($unique_group_array_values_level3 as $key_group_level3) {
                        // Output the third level group header.
                        $out .= $this->open_tag($row_tag, 'xcrud-row xcrud-grouping-row-level3') . $this->open_tag($item, '', array('colspan' => count($this->columns))) . $key_group_level3 . $this->close_tag($item) . $this->close_tag($row_tag);

                        // Iterate over the result list to find rows matching the current grouping levels.
                        foreach ($this->result_list as $key => $row) {
                            $match_level1 = ($row[$this->group_by_columns[0]['table'] . "." . $this->group_by_columns[0]['field']] == $key_group_level1);
                            $match_level2 = ($row[$this->group_by_columns[1]['table'] . "." . $this->group_by_columns[1]['field']] == $key_group_level2);
                            $match_level3 = ($row[$this->group_by_columns[2]['table'] . "." . $this->group_by_columns[2]['field']] == $key_group_level3);

                            // If the row matches the current grouping levels, render it.
                            if ($match_level1 && $match_level2 && $match_level3) {
                                $out .= $this->open_tag($row_tag, 'xcrud-row xcrud-row-' . $i);
                                // Render other cells in the row...
                                // You need to adapt this part based on your existing implementation.
                                // Render each cell in the row based on your existing logic.
                                // Example: $out .= $this->open_tag($item) . $row['column_name'] . $this->close_tag($item);
                                $out .= $this->close_tag($row_tag);
                                $i = 1 - $i; // Toggle row style
                            }
                        }
                    }
                }
            ...
            // Existing code for rendering without grouping.
            // You should not need to modify this part.
            // It remains the same as your original code.
            // Render the grid body without grouping.
            foreach ($this->result_list as $key => $row) {
                // Render each row without grouping.
                // Example: $out .= $this->open_tag($row_tag) . $this->open_tag($item) . $row['column_name'] . $this->close_tag($item) . $this->close_tag($row_tag);
            }
        }
    ...
    protected function render_grid_body_panels($row_tag = array('tag' => 'div'), $item = array('tag' => 'span'))
    {
        ...
        if(count($this->group_by_columns)>0){ //has grouping columns                
            ...
            if ($this->result_list)
            {
                foreach ($this->result_list as $key => $row)
                {
                    ...
                    if (isset($this->highlight_row))
                    {
                        foreach ($this->highlight_row as $params)
                        {
                            ...
                            if ($this->_compare($row[$params['field']], $params['operator'], $params['value']))
                            {
                                ...
                                {                   
                                    $v = $row[$field];
                                    $p = $this->replace_text_variables($params['value'], $row, true);

                                    
                                    if ($this->_compare($v, $params['operator'], $p))
                                    {
                                        $main_row_color = "$row_color";  
                                    }
                                }  
                    ...
                    $out .= $this->open_tag($row_tag, 'xcrud-row xcrud-row-' . $i,array('style' => $main_row_color));
                    if ($this->is_numbers)
                    {
                        $out .= $this->open_tag($item, 'xcrud-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) . ($key +
                            $this->start + 1) . $this->close_tag($item);
        ...
        $width_ = 100/$columns_per_row;
        $width_ = $width_ -1;
        $width_ = $width_ . "%!important";
...
/** renders action button for details view */
    ...
    protected function add_button($class = '', $icon = '')
    {
        ...
        $table_join = $this->_build_table_join();
        $where = $this->_build_where();
        echo $this->advanced_params_html;
        ...
        if($result == ""){
            ...
            $current_records = count($result);
        }

        //echo ">>>" . $this->limit_add_records;
        if($this->limit_add_records != ""){
            if($current_records >=  $this->limit_add_records){
                return "<lable class=''>Cannot add records. Maximum records allowed $this->limit_add_records</label>";
            }else{
    ...
    protected function log_user_id(){
        return $this->log_user_id;
    }
    protected function log_role_id(){
        return $this->log_role_id;
    }
