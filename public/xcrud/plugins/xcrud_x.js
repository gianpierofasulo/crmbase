...
var Xcrud = {
    ...
    current_tab: null,
    activeOption: null,
    arrowImage :'./img/select_arrow.png',	// Regular arrow
	arrowImageOver: './img/select_arrow_over.png',	// Mouse over
	arrowImageDown: './img/select_arrow_down.png',	// Mouse down
	selectBoxIds: 0,
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (data.task == "edit") {
                    ...
                    console.log("Listings Count", xcrudListingCount);
                    ...
                    console.log("Reloaded Status", reload_after);
                    if (data.task != "refresh") {
                        $(document).trigger("xcrudafterrequest", [container, data, status]);
                    }
    ...
    change_filter: function(type, container, fieldname) {
        ...
        $(container).find(".xcrud-searchdata").next(".select2-container").hide().removeClass("xcrud-search-active");
        $(container).find(".xcrud-daterange").next(".select2-container").hide().removeClass("xcrud-search-active");
        ...
        $(container).find('.xcrud-searchdata[data-fieldtype="' + fieldtype + '"]' + name_selector).next(".select2-container").show().addClass("xcrud-search-active");
        ...
        if (fieldtype == 'date') {
            ...
            $(container).find(".xcrud-daterange").next(".select2-container").show().addClass("xcrud-search-active");
        }
    },
    ...
    /*saveTestResults: function(a, b) {
        var html = "";
        for (var i = 0; i <= val; i++) {
            val_s = $("#param_" + i).val();
            label_s = $("#label_" + i).val();
        ...
        $.get(url + "index.php/add_test_results/items/" + html + "/" + id, function(data) {
            // getSelectedTests();
            alert("Saved");
        });
    },*/
    ...
    set_tab_buttons: function(tab_name) {

        if (tab_name == "Bills") {
            //$(".generatebill_btn").remove();
            //$(".tab-pane.active").append('<button style="margin-top:10px;" val="" type="button" class="btn btn-primary generatebill_btn" onclick="generateBill();">Generate Bill</button>');

            $(".makepayment_btn").remove();
            //$(".tab-pane.active").append('<button style="margin-left:10px;margin-top:10px;" type="button" class="btn btn-primary makepayment_btn" onclick="makePayment();">Pay/Generate Receipt to Selected Items</button>');
        }
        ...
        if ($(container).find('.xcrud-tabs').length) {
            if (typeof($.fn.tab) != 'undefined') {
                ...
                $(container).find('.xcrud-tabs > ul:first > li > a').on("click", function(event, ui) {
                    ...
                    console.log("TName=" + Xcrud.current_tab);

                    if (Xcrud.current_tab == 'Extras') {
                        Xcrud.initializeMultiSelect();
                    }

                    if (Xcrud.current_tab == 'Vehicle Values') {
                        //$('#myid').contentWindow.location.reload();
                        $( '#myid' ).attr( 'src', function ( i, val ) { return val; });
                    }

                    Xcrud.set_tab_buttons(Xcrud.current_tab);


                    if (Xcrud.current_tab == 'Vehicle Images') {
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/vendor/jquery.ui.widget.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/tmpl.min.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/load-image.all.min.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/canvas-to-blob.min.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '//blueimp.github.io/Gallery/js/jquery.blueimp-gallery.min.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.iframe-transport.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload-process.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload-image.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload-audio.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload-video.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload-validate.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/jquery.fileupload-ui.js',
                            dataType: 'script',
                            async: true
                        });
                        jQuery.ajax({
                            url: '../zed/fileuploader/js/main.js?v=1.0.8',
                            dataType: 'script',
                            async: true
                        });
                    }
    ...
    externalFunctions: function(container) {

        //Diagosis
        try {
            $('.diagnosis-listings, .selected-diagnosis').html($("#diagnosis_table_template").html());

            $("#search-diagnosis").on('keyup', function() {
                if ($("#search-diagnosis").val().length > 1) {
                    getAvailableDiagnosis();
                }
            });
            $(".getavailablediagnosis").on('click', function() {
                getAvailableDiagnosis();
            });
        ...
        try {
            $('.presc_selected').html($("#drugs_table_template").html());

            $("#search-drug").on('keyup', function() {
                if ($("#search-drug").val().length > 1) {
                    getAvailableDrugs();
                }
            });
            $(".getavailabledrugs").on('click', function() {
                getAvailableDrugs();
            });
        ...
        try {
            $('.dispense_selected').html($("#drugs_table_template_").html());
            getSelectedDispenseDrugs();
        } catch (e) {

        }

        try {
            $('.wi_selector, .wi_selected').html($("#wi_table_template").html());

            $("#search_ward_item").on('keyup', function() {
                if ($("#search_ward_item").val().length > 1) {
                    getAvailableWardItems();
                }
            });
            $(".getavailablewardItems").on('click', function() {
                getAvailableWardItems();
            });

            getSelectedWardItems();
        } catch (e) {
        ...
        try {
            $('.left_selector, .selected-test').html($("#table_template").html());

            $("#search-tests").on('keyup', function() {
                if ($("#search-tests").val().length > 1) {
                    getAvailableTests();
                }
            });
            $(".getavailabletests").on('click', function() {
                getAvailableTests();
            });
    ...
    initializeMultiSelect: function() {
        ...
        //$(".xcrud-input").multiselect('select', 2);
        console.log("multiselect intitalized");
    ...
    check_script: function(container) {
        ...
        if ($(elements).length) {
            console.log("script exists", $(".xcrud-callback-script").val());
...
$(document).ready(function() {
    ...
    if ($(".xcrud").length) {
        ...
        $(".xcrud").on("click", ".xcrud-search-toggle", function() {
            ...
            $(this).hide(200);
            $(this).closest(".xcrud-ajax").find(".xcrud-search").show(200);
            //$(this).closest(".xcrud-ajax").find(".xcrud-searchdata").hide().removeClass("xcrud-search-active");
            $(this).closest(".xcrud-ajax").find(".xcrud-searchdata").next(".select2-container").hide().removeClass("xcrud-search-active");
            $(this).closest(".xcrud-ajax").find(".xcrud-daterange").next(".select2-container").hide().removeClass("xcrud-search-active");
            return false;
        });
...
$(document).on("xcrudafterrequest", function(event, container) {
    ...
    Xcrud.externalFunctions(container);
    Xcrud.initializeMultiSelect();

     $(container).closest(".xcrud-ajax").find(".xcrud-searchdata").next(".select2-container").hide().removeClass("xcrud-search-active");

    //reload to the same tab
    try {
    ...
    setTimeout(function() {
        ...
        if($(container).closest(".xcrud-ajax").find(".xcrud-daterange").is(":visible")){
            $(container).closest(".xcrud-ajax").find(".xcrud-daterange").next(".select2-container").show().addClass("xcrud-search-active");
        }else{
            $(container).closest(".xcrud-ajax").find(".xcrud-daterange").next(".select2-container").hide().removeClass("xcrud-search-active");
        }
        try {
